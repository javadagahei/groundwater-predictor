...
def load_RM_GW_and_HYRAS_Data(well_code):
    pathGW = "D:\data\DATA\GWL"
    pathHYRAS = "D:\data\DATA\MD"
    pathconnect = "/"
    GWData_list = glob.glob(pathGW + pathconnect + '*.csv')
    
    # پیدا کردن فایل‌هایی که شماره چاه را در نام خود دارند
    matching_files = [f for f in GWData_list if str(well_code) in os.path.basename(f)]
    if not matching_files:
        raise ValueError(f"No file found for well code: {well_code}")
    
    file_path = matching_files[0]
    Well_ID = os.path.basename(file_path).replace('_GWdata.csv', '')
    
    GWData = pd.read_csv(file_path, 
                         parse_dates=['Date'], index_col=0, dayfirst=True, 
                         decimal='.', sep=',')
    HYRAS_file = os.path.join(pathHYRAS, Well_ID + '_HYRASdata.csv')
    HYRASData = pd.read_csv(HYRAS_file,
                            parse_dates=['Date'], index_col=0, dayfirst=True,
                            decimal='.', sep=',')
    data = pd.merge(GWData, HYRASData, how='inner', left_index=True, right_index=True)
    
    # معرفی gShift برای ورودی اضافی
    GWData_shift1 = GWData.copy()
    GWData_shift1.index = GWData_shift1.index.shift(periods=1, freq='MS')
...
def gwmodel(ini,GLOBAL_SETTINGS,X_train, Y_train,X_stop, Y_stop):
    ...
    model.add(ks.layers.Conv1D(filters=GLOBAL_SETTINGS["filters"], kernel_size=GLOBAL_SETTINGS["kernel_size"], 
                                                padding='same', activation='relu', 
                                                input_shape=(GLOBAL_SETTINGS["seq_length"], X_train.shape[2])))
    model.add(ks.layers.MaxPooling1D(padding='same'))
    model.add(ks.layers.Flatten())
    model.add(ks.layers.Dense(GLOBAL_SETTINGS["dense_size"], activation='relu'))
    model.add(ks.layers.Dense(1, activation='linear'))

    optimizer = ks.optimizers.Adam(learning_rate=GLOBAL_SETTINGS["learning_rate"], epsilon=10E-3, clipnorm=GLOBAL_SETTINGS["clip_norm"])
...
def bayesOpt_function(pp,densesize, seqlength, batchsize, filters, ET, T, Tsin):
    ...
    return bayesOpt_function_with_discrete_params(pp, densesize_int, seqlength_int, batchsize_int, filters_int, ET, T, Tsin)

#this is the real optimizer function
def bayesOpt_function_with_discrete_params(pp,densesize_int, seqlength_int, batchsize_int, filters_int, ET, T, Tsin):
    ...
    GLOBAL_SETTINGS = {
        ...
        'epochs': 30,
        'learning_rate': 1e-3,
        'test_start': pd.to_datetime('01012014', format='%d%m%Y'),
        'test_end': pd.to_datetime('01032019', format='%d%m%Y')
    }
...
def simulate_testset(pp,densesize_int, seqlength_int, batchsize_int, filters_int, ET, T, Tsin):
...
with tf.device("/cpu:0"):
    ...
    basedir = 'D:\data\DATA\CNNsh'
    os.chdir(basedir)

    well_ids = [4301, 4302, 4303, 4306, 4307, 4309, 4310, 4311, 4312, 4313, 4314, 4315, 4316, 4317, 4318, 4319, 4321, 4322, 4324, 4325, 4326]

    for idx, pp in enumerate(well_ids): # loop over all wells with index
        time_single = datetime.datetime.now()
        seed(1)
        tf.random.set_seed(1)
    
        _, Well_ID = load_RM_GW_and_HYRAS_Data(idx)
        
        # Create a directory for the current well
        well_dir = os.path.join(basedir, str(pp))
        if not os.path.exists(well_dir):
            os.makedirs(well_dir)
        
        # Change directory to the well directory
        os.chdir(well_dir)
        ...
        print("Starttime of first iteration: {}\n".format(datetime.datetime.now()), file=f) # this is not logged in json file
        utility = UtilityFunction(kind="ei", kappa=0.05, xi=0.01)
        optimizer.set_gp_params(kernel=None, alpha=1e-6)
        optimizer.maximize(
            init_points=10, # steps of random exploration
        ...
        while not beststep:
            step = step + 1
            beststep = optimizer.res[step] == optimizer.max # find the current best iteration
        ...
        ET = int(round(optimizer.max.get("params").get("ET")))
        ...
        scores, TestData, sim, obs, inimax, testresults_members, Well_ID, errors = simulate_testset(pp, densesize_int, seqlength_int, batchsize_int, filters_int, ET, T, Tsin)
        ...
        pyplot.title("CNNshift GWLt-1: " + Well_ID, size=15)
...
filters_int, densesize_int, seqlength_int, batchsize_int, ET, T, Tsin)
