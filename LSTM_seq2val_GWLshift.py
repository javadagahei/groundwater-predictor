...
def load_RM_GW_and_HYRAS_Data(i):
    pathGW = "D:\proposal-paper\data\DATA\GWData\ارسنجان"
    pathHYRAS = "D:\proposal-paper\data\DATA\MeteoData\ارسنجان"
    pathconnect = "/"
...
def bayesOpt_function(pp,hiddensize, seqlength, batchsize,ET,T,Tsin):
    ...
    return bayesOpt_function_with_discrete_params(pp, hiddensize_int, seqlength_int, batchsize_int, ET, T, Tsin)

#this is the real optimizer function
def bayesOpt_function_with_discrete_params(pp,hiddensize_int, seqlength_int, batchsize_int, ET, T, Tsin):
    ...
    GLOBAL_SETTINGS = {
        ...
        'test_start': pd.to_datetime('01012011', format='%d%m%Y'),
...
def simulate_testset(pp,hiddensize_int, seqlength_int, batchsize_int, ET, T, Tsin):
...
with tf.device("/gpu:1"):
    ...
    basedir = 'D:\proposal-paper\data\تشک بختگان\ارسنجان\خروجی\LSTM\seq2val_GWLshift'
    os.chdir(basedir)
    
    for pp in range(0,1): #loop over all wells
        ...
        # load_logs(optimizer, logs=["./logs_LSTM_S"+Well_ID+".json"]);
        # print("\nExisting optimizer is already aware of {} points.".format(len(optimizer.space)))
        
        # Saving progress
        logger = JSONLogger(path="./logs_LSTM_"+Well_ID+".json")
        optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)
        
        # random exploration as a start
        f = open('./timelog_LSTM_'+Well_ID+'.txt', "w")
        ...
        scores, TestData, sim, obs, inimax, testresults_members, Well_ID, errors = simulate_testset(pp,hiddensize_int, seqlength_int, batchsize_int, ET, T, Tsin)
        t2_test = datetime.datetime.now()
        f = open('./timelog_LSTM_'+Well_ID+'.txt', "a")
        ...
        pyplot.title("LSTM: "+Well_ID, size=15)
...
batchsize = {:d}\nET = {:d}\nT = {:d}\nTsin = {:d}""".format(scores.NSE[0],scores.R2[0],
        ...
        hiddensize_int,seqlength_int,batchsize_int,ET,T,Tsin)
        
        pyplot.figtext(0.856, 0.4, s, bbox=dict(facecolor='white'))
        pyplot.savefig(Well_ID+'_testset_LSTM.png', dpi=300)
        pyplot.show()
        
        # print log summary file
        f = open('./log_summary_LSTM_'+Well_ID+'.txt', "w")
        print("\nBEST:\n\n"+s+"\n", file = f)
        ...
        np.savetxt('./ensemble_member_errors_LSTM_'+Well_ID+'.txt',errors,delimiter=';',header = 'NSE;R²;RMSE;rRMSE;Bias;rBias')
