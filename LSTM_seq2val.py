# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 13:56:51 2020
...
def load_RM_GW_and_HYRAS_Data(i):
    pathGW = "D:\proposal-paper\data\DATA\GWData"
    pathHYRAS = "D:\proposal-paper\data\DATA\MeteoData"
    pathconnect = "/"
...
def gwmodel(ini,GLOBAL_SETTINGS,X_train, Y_train,X_stop, Y_stop):
    ...
    optimizer = ks.optimizers.Adam(learning_rate=GLOBAL_SETTINGS["learning_rate"], epsilon=10E-3,
...
with tf.device("/cpu:0"): #type cpu to use cpu instead of gpu
    ...
    for pp in range(0,1): #loop over all wells
        ...
        # load_logs(optimizer, logs=["./logs_LSTM_"+Well_ID+".json"]);
        ...
        scores, TestData, sim, obs, inimax, testresults_members, Well_ID, errors = simulate_testset(pp,hiddensize_int, seqlength_int, batchsize_int, rH, T, Tsin)
...
batchsize = {:d}\nrH = {:d}\nT = {:d}\nTsin = {:d}""".format(scores.NSE[0],scores.R2[0],
        ...
        hiddensize_int,seqlength_int,batchsize_int,rH,T,Tsin)
        ...
        np.savetxt('./ensemble_member_errors_LSTM_'+Well_ID+'.txt',errors,delimiter=';',header = 'NSE;R²;RMSE;rRMSE;Bias;rBias;PI', fmt = '%.4f')
